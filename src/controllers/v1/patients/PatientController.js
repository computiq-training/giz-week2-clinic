...
const getHistoryOfPatient = async (req, res) => {
  ...
  // TO-DO
  const p = await Patient.findById(id).populate("history");
...
const createPatient = async (req, res) => {
  ...
  try {
    const routedObject = await p.save();
    return res.status(200).json(success(200, routedObject, "OK"));
  } catch (_error) {
...
const searchPatients = async (req, res) => {
  ...
  if (data) return res.status(200).json(success(200, data, "OK"));
...
const newHistory = async (req, res) => {
  ...
  if (!p) return res.status(404).json(error(404, {}, "No Data"));
  let h = new History(req.body);
  let h2 = await h.save();
  p.history.push(h2._id);
  p.save();
